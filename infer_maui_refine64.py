...
def main():
    ...
    for row,col in selected:
        ...
        for dy in (0,STEP,2*STEP,3*STEP):
            for dx in (0,STEP,2*STEP,3*STEP):
    ...
    ax.set_title('64x64 refinement within coarse Wildfire tiles (resized to 256 for model)'); ax.axis('off'); fig.savefig(OUT/'refined_wildfire_heatmap.png',dpi=220,bbox_inches='tight'); plt.close(fig)
